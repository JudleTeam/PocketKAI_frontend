...
import styles from './FullLessonCard.module.scss'
        ...
        >
            ...
            >
                <Text 
                    ...
                    fontWeight='bold'
                    fontSize='18px'
                >{sliceLessonName(lesson.discipline.name)}</Text>
                <Text
                    color='gray.400'
                    fontWeight='medium'
                    fontSize='22px'
                >{lesson.start_time?.slice(0, 5)} {lesson.end_time && '-'}{' '} {lesson.end_time?.slice(0, 5)}</Text>
            </VStack>
            <VStack 
                w='40%'
                alignItems='center'
            ...
            >
              ...
              LessonTypes[lesson.parsed_lesson_type]}</Text>
                <Text
                fontWeight='medium'
                fontSize='14px'
                w='60%'
                textAlign='center'
                >{getLessonBuilding(lesson.building_number, lesson.audience_number)}</Text>
            </VStack>
            <VStack
            alignItems='center'

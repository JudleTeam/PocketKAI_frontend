...
export function HiddenLessons() {
  ...
  const { hiddenLessons, addHiddenLesson, deleteHiddenLesson } = useSchedule();
      ...
      >
        {weekDaysOrder.map((dayName) =>
          groupedLessons[dayName] ? (
            <Box key={dayName}>
              <Box w="100%" fontSize="18px" fontWeight="medium" py="5px">
                <Box
                  as="span"
                  borderRadius={3}
                  py={0.5}
              ...
              {groupedLessons[dayName].sort((a, b) => {
                ...
                >
                  <Box w="50%">
                    <Popover
                      ...
                      onOpen={() => lesson.id && setOpenPopoverId(lesson.id)}
  ...
  <MenuList>
    {lesson.type_hide !== 'always' && (
      <MenuItem
        onClick={() => {
          addHiddenLesson({ ...lesson, type_hide: 'always' });
        }}
        display="flex"
        alignItems="center"
        justifyContent="space-between"
      >
        <Text color={mainTextColor}>Скрыть навсегда</Text>
      </MenuItem>
    )}
    {lesson.type_hide !== 'odd' && (
      <MenuItem
        onClick={() => {
          addHiddenLesson({ ...lesson, type_hide: 'odd' });
        }}
        display="flex"
        alignItems="center"
        justifyContent="space-between"
    ...
    {lesson.type_hide !== 'even' && (
      <MenuItem
        onClick={() => {
          addHiddenLesson({ ...lesson, type_hide: 'even' });
        }}
        display="flex"
        alignItems="center"
        justifyContent="space-between"

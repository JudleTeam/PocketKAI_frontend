...
export function IdleMessage() {
  ...
  const isTeacherOrFull =
    location.pathname === '/teachers' ||
    (fullScheduleView === 'week' && location.pathname === '/schedule/full');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <Box
        position={'fixed'}
        top={isTeacherOrFull ? '24dvh' : '40dvh'}
      ...
      >
        <Box display="flex" flexDir="column" alignItems="center" gap="5px">
          ...
          <Button
            as={DialogTrigger}
            onClick={() => {
              sendEvent(AnalyticsEvent.mainModalOpen, {
        ...
        <Box display="flex" flexDir="column" alignItems="center" gap="5px">
          <Text
            color={secondaryDayNameColor}
            textAlign={'center'}
            fontSize={'18px'}
            w="75%"
          >
            Также следите за новостями в Telegram-канале:
          </Text>
          <Text
            as={Link}
            onClick={() => {
